...
| 6 | `main.py`，rollout 客户端 | rig venv + ROS |
...
```bash
# 终端 2
cd $OPENPI/examples/sharpa/rollout
...
$V/.venv/bin/python -u $V/openarm_vr_teleop.py \
    ...
    --hand --telemetry --switch --grav-ff-mode effort
```
...
cd $OPENPI/examples/sharpa/rollout
$V/.venv/bin/python main.py --prompt "pick up the egg"
```
...
- `--hand-norm-stats` **每次都带**。它把手指包络从固件的 ±π/2 收到策略实际训练过的逐关节范围
  ...
  并打 warning。
- `--record` 也每次都带。没有飞行记录，调一次失败的 rollout 全靠猜。
...
客户端有三层，触发时 rollout 停止，节点保持在**最后一个通过检查的目标**：

| 触发 | 日志 | 看的是什么 |
|---|---|---|
| 单个动作腕部位移 > 20 mm 或转角 > 5.7° | `SAFETY STOP: single action moves the wrist ...` | **一步**。训练数据里单步上限是 4.6 mm / 1.3°，默认限值约 4 倍。这一步既没被积分也没被发出去 |
| 累计离开啮合点 > 45 cm 或 1.8 rad | `SAFETY STOP: ... left the envelope` | **累积**。逐步都在分布内的慢漂移只有这层看得见 |
| 任一观测源超过 0.25 s 不更新 | `stale observation (max_age=0.25s): {...}` | 不拿冻结的画面继续喂策略 |

两层都需要：单步护栏看不见 4 mm/step 的漂移（每个动作都正常），而累积包络要一百步、三秒才发火。
